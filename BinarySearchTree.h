...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int size_impl(const Node *node) {
      ...
      if (empty_impl(node)) {
          ...
          size = 1 + size_impl(node -> left);
          size = size + size_impl(node -> right);
      }
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
      ...
      if (node == nullptr) {
          ...
          findNode = find_impl(node -> left, query, less);
      }
      else if (less(node -> datum, query)) {
          findNode = find_impl(node -> right, query, less);
      }
      else {
          return node;
      }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
      ...
      if (less(val, node->datum)) {
            Node *nextNode = min_greater_than_impl(node->left, val, less);
            if (nextNode) {
                return nextNode;
            }
            else {
